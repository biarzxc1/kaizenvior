...
def auto_create_pages():
    ...
    # Select account
    acc_choice = input(f" {W}[{W}➤{W}]{RESET} {C}SELECT ACCOUNT NUMBER (0 to cancel) {W}➤{RESET} ").strip()
    ...
    while True:
        ...
        print(LINE)
        print(f" {Y}Account: {M}{selected_acc['name']}{RESET}")
        print(f" {Y}Page Name: {G}{page_name}{RESET}")
        print(LINE)
        
        confirm = input(f" {W}[{W}➤{W}]{RESET} {Y}Create this page? (Y/N) {W}➤{RESET} ").strip().upper()
...
async def auto_share_page_mode(link):
    ...
    async with aiohttp.ClientSession() as session:
        ...
        print(LINE)
        print(f" {W}[{RESET}{BG_G}{W}1{RESET}{W}]{RESET} {G}Use paired accounts from database{RESET}")
        print(f" {W}[{RESET}{BG_C}{W}2{RESET}{W}]{RESET} {C}Enter cookies manually (auto-convert to tokens){RESET}")
        print(LINE)
