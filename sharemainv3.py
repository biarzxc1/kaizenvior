...
def auto_create_pages():
    ...
    # Display accounts with styled format
    for i, acc in enumerate(accounts, 1):
        print(f" {W}[{RESET}{BG_C}{W}{i:02d}{RESET}{BG_C}{Y}/{RESET}{BG_C}{W}{chr(64+i)}{RESET}{W}]{RESET} {M}{acc['name']}{RESET} {W}({C}UID: {acc['uid']}{W}){RESET}")
    
    print(LINE)
    print(f" {C}[!] SELECT ACCOUNT TO USE FOR PAGE CREATION{RESET}")
    print(LINE)
    
    choice = input(f" {W}[{RESET}{BG_G}{W}➤{RESET}{W}]{RESET} {C}SELECT ACCOUNT NUMBER {W}[{RESET}{BG_R}{W}0{RESET}{W}]{RESET} {R}to cancel {W}➤{RESET} ").strip()
    ...
    count_input = input(f" {W}[{RESET}{BG_C}{W}?{RESET}{W}]{RESET} {C}NUMBER OF PAGES (1-50) {W}➤{RESET} ").strip()
    
    if not count_input.isdigit() or int(count_input) < 1 or int(count_input) > 50:
        print(f" {R}[ERROR] Please enter a number between 1 and 50{RESET}")
        time.sleep(1)
        return
    
    page_count = int(count_input)
    
    # Collect ALL page names FIRST before creating any
    page_names = []
    for i in range(1, page_count + 1):
        refresh_screen()
        print(f" {C}[ENTER PAGE NAMES] - {i-1}/{page_count} completed{RESET}")
        print(LINE)
        print(f" {Y}Account: {M}{selected_account['name']}{RESET}")
        print(f" {Y}Total Pages: {C}{page_count}{RESET}")
        print(LINE)
        
        # Show already entered names
        if page_names:
            print(f" {G}[PAGES ENTERED]{RESET}")
            for j, pname in enumerate(page_names, 1):
                print(f" {W}[{RESET}{BG_G}{W}PAGE {j}{RESET}{W}]{RESET} {Y}{pname}{RESET}")
            print(LINE)
        
        page_name = input(f" {W}[{RESET}{BG_C}{W}PAGE {i}{RESET}{W}]{RESET} {C}Enter page name {W}➤{RESET} ").strip()
    ...
    # Confirm ALL pages before creating
    refresh_screen()
    print(f" {Y}[CONFIRM PAGE CREATION]{RESET}")
    print(LINE)
    print(f" {Y}Account: {M}{selected_account['name']}{RESET}")
    print(f" {Y}Total Pages: {C}{page_count}{RESET}")
    print(f" {Y}Cooldown: {R}5 minutes between each page{RESET}")
    print(LINE)
    print(f" {G}[PAGES TO CREATE]{RESET}")
    for i, pname in enumerate(page_names, 1):
        print(f" {W}[{RESET}{BG_G}{W}PAGE {i}{RESET}{W}]{RESET} {Y}{pname}{RESET}")
    print(LINE)
    
    confirm = input(f" {W}[{RESET}{BG_Y}{W}?{RESET}{W}]{RESET} {Y}Create all these pages? {W}[{RESET}{BG_G}{W}Y{RESET}{W}]{RESET}{G}es{RESET} / {W}[{RESET}{BG_R}{W}N{RESET}{W}]{RESET}{R}o{RESET} {W}➤{RESET} ").strip().upper()
    ...
    for i, page_name in enumerate(page_names, 1):
        # If not first page, apply cooldown
        if i > 1:
            refresh_screen()
            print(f" {Y}[COOLDOWN ACTIVE]{RESET}")
            print(LINE)
            print(f" {Y}Progress: {G}{i-1}/{page_count}{Y} pages created{RESET}")
            print(f" {Y}Next: {C}{page_name}{RESET}")
            print(LINE)
            ...
            for remaining in range(300, 0, -1):
                ...
                sys.stdout.write(f"\r {C}[Creating next page in {timer}]{RESET} {W}Please wait...{RESET}")
                sys.stdout.flush()
                time.sleep(1)
        ...
        # Create the page
        refresh_screen()
        print(f" {G}[!] CREATING PAGE {i}/{page_count}...{RESET}")
        print(LINE)
        print(f" {Y}Page Name: {C}{page_name}{RESET}")
        print(LINE)
        nice_loader("CREATING")
        
        result = create_facebook_page_api(token, page_name)
        ...
        if result['success']:
            ...
            print(f" {G}[SUCCESS] Page {i}/{page_count} created!{RESET}")
            print(LINE)
            print(f" {Y}Name: {G}{page_name}{RESET}")
            print(f" {Y}Page ID: {C}{page_id}{RESET}")
            print(f" {Y}Progress: {G}{len(created_pages)}/{page_count}{RESET}")
            print(LINE)
            
            if i < page_count:
                print(f" {C}[!] Next page in 5 minutes...{RESET}")
            else:
                print(f" {G}[!] ALL PAGES CREATED!{RESET}")
            print(LINE)
        else:
            failed_pages.append({
                'number': i,
            ...
            print(f" {R}[FAILED] Page {i}/{page_count} creation failed!{RESET}")
            print(LINE)
            print(f" {Y}Name: {R}{page_name}{RESET}")
            print(f" {R}Reason: {result['message']}{RESET}")
            print(f" {Y}Progress: {Y}{len(created_pages)}/{page_count}{RESET}")
            print(LINE)
            
            if "too many Pages" in result['message']:
                print(f" {R}[!] Facebook has limited your page creation{RESET}")
                print(f" {Y}[!] Cannot continue with remaining pages{RESET}")
                print(LINE)
                break
    ...
    if created_pages:
        ...
        for page in created_pages:
            print(f" {W}[{RESET}{BG_G}{W}PAGE {page['number']}{RESET}{W}]{RESET} {G}{page['name']}{RESET} {W}({C}ID: {page['page_id']}{W}){RESET}")
        print(LINE)
    
    if failed_pages:
        print(f" {R}[FAILED PAGES]{RESET}")
        for page in failed_pages:
            print(f" {W}[{RESET}{BG_R}{W}PAGE {page['number']}{RESET}{W}]{RESET} {R}{page.get('name', 'N/A')}{RESET}")
...
async def share_loop(session, tk, ck, post, page_id, target=None, display_mode='detailed', total_target=None):
    """
    ...
    target: shares this page should do before stopping
    total_target: TOTAL target across ALL pages (for display only)
    """
    ...
    while True:
        ...
        if target and page_count >= target:
            ...
            if display_mode == 'minimal':
                sys.stdout.write(f"\r {G}[TARGET REACHED]{RESET} {Y}[{page_count}/{target}]{RESET} {C}[{page_id}]{RESET}                         \n")
                sys.stdout.flush()
            else:
        ...
        try:
            ...
            if is_success:
                ...
                if display_mode == 'minimal':
                    # Minimal display - format: [SUCCESS] [TOTALCOUNT/TOTALTARGET] [PAGE ID]
                    if total_target:
                        sys.stdout.write(f"\r {G}[SUCCESS]{RESET} {Y}[{current_success_count}/{total_target}]{RESET} {C}[{page_id}]{RESET}                    ")
                    else:
                        sys.stdout.write(f"\r {G}[SUCCESS]{RESET} {Y}[{current_success_count}]{RESET} {C}[{page_id}]{RESET}                    ")
                    sys.stdout.flush()
                else:
                ...
                if consecutive_block_count == 1:
                    ...
                    if display_mode == 'minimal':
                        sys.stdout.write(f"\r {Y}[RETRY]{RESET} {C}[{page_id}]{RESET} {W}Switching...{RESET}                              ")
                        sys.stdout.flush()
                    else:
                    ...
                    # THIS ACCOUNT IS BLOCKED - Stop only this account, not all
                    current_published_status = 0
                    
                    if display_mode == 'minimal':
                        sys.stdout.write(f"\r {R}[BLOCKED]{RESET} {C}[{page_id}]{RESET} {W}Stopping this account...{RESET}                    \n")
                        sys.stdout.flush()
                    else:
                        print(f" {R}[BLOCKED]{RESET} {W}|{RESET} {M}{current_time}{RESET} {W}|{RESET} {B}{page_id}{RESET} {W}|{RESET} {R}This account is blocked - stopping{RESET}")
                    
                    # Sleep for 30 minutes then retry (only this account)
                    if display_mode == 'minimal':
                        sys.stdout.write(f"\r {Y}[PAUSED]{RESET} {C}[{page_id}]{RESET} {W}Waiting 30 min...{RESET}                           ")
                        sys.stdout.flush()
                    ...
                    if display_mode == 'minimal':
                        sys.stdout.write(f"\r {G}[RETRY]{RESET} {C}[{page_id}]{RESET} {W}Resuming this account...{RESET}                    ")
                        sys.stdout.flush()
                    else:
                        print(f" {G}[RETRY]{RESET} {W}|{RESET} {M}{datetime.datetime.now().strftime('%H:%M:%S')}{RESET} {W}|{RESET} {B}{page_id}{RESET} {W}|{RESET} {G}Retrying after pause{RESET}")
                    continue
...
async def auto_share_page_mode(link):
    ...
    async with aiohttp.ClientSession() as session:
        ...
        if choice in ['1', 'A']:
            ...
            refresh_screen()
            print(f" {C}[!] MANUAL COOKIE INPUT & AUTO-PAIR{RESET}")
            print(LINE)
            print(f" {Y}⚠ WARNING: Do not spam cookies!{RESET}")
            print(f" {Y}⚠ Each cookie will be auto-converted to EAAAAU token{RESET}")
            print(f" {G}✓ Successfully converted accounts will be paired to database{RESET}")
            print(LINE)
            ...
            while True:
                ...
                print(f" {Y}Type {W}[{RESET}{BG_G}{W}DONE{RESET}{W}]{RESET} when finished or {W}[{RESET}{BG_R}{W}CANCEL{RESET}{W}]{RESET} to abort.{RESET}")
                print(LINE)
                
                cookie_input = input(f" {W}[{RESET}{BG_C}{W}{cookie_count + 1:02d}{RESET}{BG_C}{Y}/{RESET}{BG_C}{W}C{RESET}{W}]{RESET} {C}COOKIE {W}➤{RESET} ").strip()
                ...
                if eaaaau_token:
                    ...
                    # Auto-pair to database
                    refresh_screen()
                    print(f" {G}[!] PAIRING ACCOUNT TO DATABASE...{RESET}")
                    nice_loader("PAIRING")
                    
                    pair_status, pair_response = api_request("POST", "/user/accounts/add", {
                        "cookie": cookie_input,
                        "token": eaaaau_token
                    })
                    ...
                    refresh_screen()
                    print(f" {G}[SUCCESS] Cookie #{cookie_count} converted & paired!{RESET}")
                    print(LINE)
                    print(f" {Y}Name: {M}{name}{RESET}")
                    print(f" {Y}UID: {C}{uid}{RESET}")
                    
                    if pair_status == 200 and pair_response.get('success'):
                        print(f" {G}✓ Paired to database successfully{RESET}")
                    else:
                        print(f" {Y}⚠ Added to session but database pairing failed{RESET}")
        ...
        print(LINE)
        print(f" {Y}Total pages found: {G}{len(list_pages)}{RESET}")
        print(LINE)
        print(f" {Y}How many TOTAL shares do you want?{RESET}")
        print(f" {Y}(This will be distributed across ALL {G}{len(list_pages)}{Y} pages){RESET}")
        print(f" {Y}Leave empty for unlimited (continuous sharing){RESET}")
        print(LINE)
        
        target_input = input(f" {W}[{W}➤{W}]{RESET} {C}TOTAL TARGET (empty for unlimited) {W}➤{RESET} ").strip()
        
        total_target_count = None
        target_per_page = None
        
        if target_input:
            try:
                total_target_count = int(target_input)
                if total_target_count <= 0:
                    print(f" {R}[ERROR] Target must be greater than 0{RESET}")
                    time.sleep(1)
                    return
                
                # Calculate target per page (distribute evenly)
                target_per_page = total_target_count // len(list_pages)
                
                if target_per_page == 0:
                    print(f" {R}[ERROR] Target too small for {len(list_pages)} pages. Minimum: {len(list_pages)}{RESET}")
                    time.sleep(1)
                    return
        ...
        if total_target_count:
            print(f" {Y}Total Target: {C}{total_target_count} shares{RESET}")
            print(f" {Y}Per Page Target: {M}~{target_per_page} shares{RESET}")
            print(f" {Y}Note: {W}Will stop at exactly {C}{total_target_count}{W} total shares{RESET}")
        else:
            print(f" {Y}Target: {G}UNLIMITED{RESET}")
        ...
        for page in list_pages:
            task = asyncio.create_task(share_loop(
                ...
                post, 
                page["page_id"],
                target_per_page,       # Pass per-page target
                display_mode,          # Pass display mode
                total_target_count     # Pass TOTAL target for display
            ))
            tasks.append(task)
        ...
        try:
            ...
            if time.time() - last_token_renewal >= 300:
                ...
                if new_token:
                    ...
                    if display_mode == 'minimal':
                        sys.stdout.write(f"\r {Y}[TOKEN RENEWED]{RESET} {C}[{account_name[:20]}]{RESET}                                   ")
                        sys.stdout.flush()
                        time.sleep(1)
                    else:
                        now = datetime.datetime.now()
            ...
            if is_success:
                ...
                if display_mode == 'minimal':
                    # Minimal display - format: [SUCCESS] [COUNT] [ACCOUNT NAME]
                    sys.stdout.write(f"\r {G}[SUCCESS]{RESET} {Y}[{current_count}]{RESET} {C}[{account_name[:20]}]{RESET}                    ")
                    sys.stdout.flush()
                else:
                    # Detailed display - full logs
                ...
                if failed_consecutive >= 3 and failed_consecutive < 6:
                    if display_mode == 'minimal':
                        sys.stdout.write(f"\r {Y}[RENEWING...]{RESET} {C}[{account_name[:20]}]{RESET}                                   ")
                        sys.stdout.flush()
                    else:
                    ...
                    if new_token:
                        ...
                        if display_mode == 'minimal':
                            sys.stdout.write(f"\r {G}[TOKEN RENEWED]{RESET} {C}[{account_name[:20]}]{RESET}                                   ")
                            sys.stdout.flush()
                            time.sleep(1)
                        else:
                ...
                elif failed_consecutive >= 6:
                    # This account is blocked - pause only this account for 30 minutes
                    if display_mode == 'minimal':
                        sys.stdout.write(f"\r {R}[BLOCKED]{RESET} {C}[{account_name[:20]}]{RESET} {W}Pausing 30min...{RESET}                      \n")
                        sys.stdout.flush()
                    else:
                        print(f" {R}[BLOCKED]{RESET} {W}|{RESET} {M}{current_time}{RESET} {W}|{RESET} {B}{account_name}{RESET} {W}|{RESET} {R}Account blocked - pausing for 30 minutes{RESET}")
                    
                    await asyncio.sleep(1800)  # 30 minutes
                    failed_consecutive = 0
                    
                    if display_mode == 'minimal':
                        sys.stdout.write(f"\r {G}[RETRY]{RESET} {C}[{account_name[:20]}]{RESET} {W}Resuming...{RESET}                              ")
                        sys.stdout.flush()
                    else:
                        print(f" {G}[RETRY]{RESET} {W}|{RESET} {M}{datetime.datetime.now().strftime('%H:%M:%S')}{RESET} {W}|{RESET} {B}{account_name}{RESET} {W}|{RESET} {G}Retrying after pause{RESET}")
