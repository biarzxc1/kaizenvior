...
def G(text): return gradient(str(text), 'green')
def R(text): return gradient(str(text), 'red')
def C(text): return gradient(str(text), 'cyan')
def M(text): return gradient(str(text), 'blue')
...
def normalize_facebook_url(url):
    ...
    if not url.startswith('facebook.com'):
        if '/' not in url: url = 'facebook.com/' + url
    return url
...
def show_banner():
    ...
    if user_data:
        ...
        if plan == 'inactive':
            ...
            plan_display = M('[ MAX+ ] - ' + expiry) if expiry else M('[ MAX+ LIFETIME ]')
        else:
            plan_display = M('[ MAX ] - ' + expiry) if expiry else M('[ MAX LIFETIME ]')
...
def show_menu():
    if not user_token:
        ...
        print(f" {W}[03/C]{RESET} {G('MANAGE COOKIES                   — [V1]')}")
        print(f" {W}[04/D]{RESET} {C('MANAGE TOKENS                    — [V2]')}")
        print(f" {W}[05/E]{RESET} {M('MY STATS                         — [STATISTICS]')}")
        print(f" {W}[06/F]{RESET} {G('ADMIN PANEL                      — [MANAGEMENT]')}")
        ...
        print(f" {W}[01/A]{RESET} {G('AUTO SHARE V1                    — [NORM]')}")
        print(f" {W}[02/B]{RESET} {G('MANAGE COOKIES                   — [V1]')}")
        print(f" {W}[03/C]{RESET} {M('MY STATS                         — [STATISTICS]')}")
        print(f" {W}[00/X]{RESET} {R('LOGOUT')}")
    print(LINE)
...
def api_request(method, endpoint, data=None, use_token=True):
    ...
    try:
        if method == "GET": response = requests.get(url, headers=headers, timeout=15)
        elif method == "POST": response = requests.post(url, json=data, headers=headers, timeout=15)
        elif method == "PUT": response = requests.put(url, json=data, headers=headers, timeout=15)
        elif method == "DELETE": response = requests.delete(url, headers=headers, timeout=15)
        else: return None, "Invalid method"
        return response.status_code, response.json()
    except requests.exceptions.ConnectionError: return None, "Cannot connect to server"
    except requests.exceptions.Timeout: return None, "Request timeout"
...
def do_register():
    ...
    if confirm == 'N':
        country = input(f" {W}[➤]{RESET} {C('ENTER COUNTRY')}   {W}➤{RESET} ").strip() or 'Unknown'
...
def show_stats():
    ...
    if status == 200 and response.get('success'):
        ...
        print(LINE)
        print(f" {C('Username:')}        {M(stats.get('username', 'N/A').upper())}")
...
def manage_cookies():
    while True:
        ...
        if choice == '1': view_cookies()
        elif choice == '2': add_cookie()
        elif choice == '3': delete_cookie()
        elif choice == '4': delete_all_cookies()
...
def view_cookies():
    ...
    if status == 200 and response.get('success'):
        ...
        refresh_screen()
        print(f" {G('[COOKIES] Total: ' + str(len(cookies)))}")
        print(LINE)
        if not cookies:
            print(f" {C('No cookies stored yet.')}")
        else:
            for i, cookie in enumerate(cookies, 1):
                print(f" {W}[{str(i).zfill(2)}]{RESET} {M(cookie['name'])} {W}(UID: {C(cookie['uid'])}){RESET}")
...
def add_cookie():
    ...
    if status == 200 and response.get('success'):
        ...
        if user_data: user_data['cookieCount'] = response.get('totalCookies', 0)
    else:
...
def delete_cookie():
    ...
    print(LINE)
    choice = input(f" {W}[➤]{RESET} {C('NUMBER (0=cancel)')}  {W}➤{RESET} ").strip()
    if not choice or choice == '0': return
    try:
        index = int(choice) - 1
        if 0 <= index < len(cookies):
            show_loader("DELETING")
            status, response = api_request("DELETE", "/user/cookies/" + cookies[index]['id'])
            if status == 200 and response.get('success'):
                print(f" {G('[SUCCESS] Cookie deleted!')}")
                if user_data: user_data['cookieCount'] = response.get('totalCookies', 0)
            else: print(f" {R('[ERROR] Failed to delete cookie')}")
        else: print(f" {R('[ERROR] Invalid selection')}")
    except ValueError: print(f" {R('[ERROR] Invalid input')}")
    input(f" {G('[PRESS ENTER TO CONTINUE]')}")
...
def delete_all_cookies():
    ...
    if status == 200 and response.get('success'):
        ...
        if user_data: user_data['cookieCount'] = 0
    else: print(f" {R('[ERROR] Failed to delete cookies')}")
    input(f" {G('[PRESS ENTER TO CONTINUE]')}")
...
def manage_tokens():
    while True:
        ...
        print(LINE)
        choice = input(f" {W}[➤]{RESET} {C('CHOICE')}          {W}➤{RESET} ").strip()
        if choice == '1': view_tokens()
        elif choice == '2': add_token()
        elif choice == '3': delete_token()
        elif choice == '4': delete_all_tokens()
        elif choice == '0': return
...
def view_tokens():
    ...
    if status == 200 and response.get('success'):
        ...
        refresh_screen()
        print(f" {G('[TOKENS] Total: ' + str(len(tokens)))}")
        print(LINE)
        if not tokens:
            print(f" {C('No tokens stored yet.')}")
        else:
            for i, token in enumerate(tokens, 1):
                print(f" {W}[{str(i).zfill(2)}]{RESET} {M(token['name'])} {W}(UID: {C(token['uid'])}){RESET}")
                print(f"      Status: {G(token['status'])} | Added: {G(token['addedAt'])}")
                print(LINE)
    else: print(f" {R('[ERROR] Failed to load tokens')}")
    input(f" {G('[PRESS ENTER TO CONTINUE]')}")
...
def add_token():
    ...
    if status == 200 and response.get('success'):
        ...
        print(f" {C('Account Name:')}    {M(response.get('name', 'Unknown'))}")
        print(f" {C('Account UID:')}     {C(response.get('uid', 'Unknown'))}")
        if user_data: user_data['tokenCount'] = response.get('totalTokens', 0)
    elif status == 403: print(f" {R('[ERROR] MAX+ plan required')}")
    else:
        error_msg = response if isinstance(response, str) else response.get('message', 'Failed')
        print(f" {R('[ERROR]')} {R(error_msg)}")
    print(LINE)
...
def delete_token():
    ...
    try:
        ...
        if 0 <= index < len(tokens):
            show_loader("DELETING")
            status, response = api_request("DELETE", "/user/tokens/" + tokens[index]['id'])
            if status == 200 and response.get('success'):
                print(f" {G('[SUCCESS] Token deleted!')}")
                if user_data: user_data['tokenCount'] = response.get('totalTokens', 0)
            else: print(f" {R('[ERROR] Failed to delete token')}")
        else: print(f" {R('[ERROR] Invalid selection')}")
    except ValueError: print(f" {R('[ERROR] Invalid input')}")
    input(f" {G('[PRESS ENTER TO CONTINUE]')}")
...
def delete_all_tokens():
    ...
    if status == 200 and response.get('success'):
        ...
        if user_data: user_data['tokenCount'] = 0
    else: print(f" {R('[ERROR] Failed to delete tokens')}")
    input(f" {G('[PRESS ENTER TO CONTINUE]')}")
...
def admin_panel():
    while True:
        ...
        print(f" {W}[4]{RESET} {R('DELETE ALL USERS')}")
        print(f" {W}[5]{RESET} {G('DASHBOARD STATS')}")
        print(f" {W}[0]{RESET} {C('BACK TO MENU')}")
        print(LINE)
        choice = input(f" {W}[➤]{RESET} {C('CHOICE')}          {W}➤{RESET} ").strip()
        if choice == '1': admin_view_users()
        elif choice == '2': admin_activate_plan()
        elif choice == '3': admin_delete_user()
        elif choice == '4': admin_delete_all_users()
        elif choice == '5': admin_dashboard()
        elif choice == '0': return
...
def admin_view_users():
    ...
    if status == 200 and response.get('success'):
        ...
        refresh_screen()
        print(f" {G('[ALL USERS] Total: ' + str(len(users)))}")
        print(LINE)
        for i, user in enumerate(users, 1):
            admin_badge = " " + M('[ADMIN]') if user.get('isAdmin') else ""
            plan = user.get('plan', 'inactive')
            if plan == 'inactive': plan_status = R('INACTIVE')
            elif user.get('isLifetime'): plan_status = M(plan.upper() + ' LIFETIME')
            elif user.get('planExpiry') == 'Expired': plan_status = R('EXPIRED')
            else: plan_status = G(user.get('planExpiry', 'Active'))
            plan_name = 'MAX+' if plan == 'maxplus' else plan.upper()
            print(f" {W}[{str(i).zfill(2)}]{RESET} {C(user['username'].upper())}{admin_badge}")
            ...
            print(LINE)
    else: print(f" {R('[ERROR] Failed to load users')}")
    input(f" {G('[PRESS ENTER TO CONTINUE]')}")
...
def admin_activate_plan():
    ...
    for i, user in enumerate(users, 1):
        plan = user.get('plan', 'inactive')
        if plan == 'inactive': status_str = R('INACTIVE')
        elif user.get('isLifetime'): status_str = M('LIFETIME')
        else: status_str = G(user.get('planExpiry', 'Active'))
        print(f" {W}[{i}]{RESET} {C(user['username'].upper())} - {status_str}")
    print(LINE)
    choice = input(f" {W}[➤]{RESET} {C('USER # (0=cancel)')}  {W}➤{RESET} ").strip()
    if not choice or choice == '0': return
    try:
        index = int(choice) - 1
    ...
    if plan_choice not in plan_map: return
    ...
    if duration_choice not in duration_map: return
    duration = duration_map[duration_choice]
    show_loader("ACTIVATING PLAN")
    status, response = api_request("PUT", "/admin/users/" + selected_username + "/plan", {"plan": selected_plan, "duration": duration})
    if status == 200 and response.get('success'):
        ...
        try:
            ...
            if success:
                ...
                if display_mode == 'minimal':
                    sys.stdout.write('\r ' + G('[V1 SUCCESS — ' + str(current_count) + ']') + ' | UID: ' + uid + '              ')
                    sys.stdout.flush()
                else:
...
async def share_worker_v2_fast(session, token, post_id, uid, display_mode):
    ...
    while True:
        try:
            ...
            if success:
                ...
                if display_mode == 'minimal':
                    sys.stdout.write('\r ' + G('[V2 SUCCESS — ' + str(current_count) + ']') + ' | UID: ' + uid + '              ')
                    sys.stdout.flush()
                else:
                    timestamp = datetime.datetime.now().strftime('%H:%M:%S')
...
def select_display_mode():
    ...
    while True:
        choice = input(f" {W}[➤]{RESET} {C('CHOICE')}          {W}➤{RESET} ").strip()
        if choice == '1': return 'minimal'
        elif choice == '2': return 'detailed'
...
def select_cookies_for_sharing():
    ...
    if status != 200 or not response.get('success'):
        print(f" {R('[ERROR] Failed to load cookies')}")
        input(f" {G('[PRESS ENTER TO CONTINUE]')}"); return None
    cookies = response.get('cookies', [])
    if not cookies:
    ...
    for i, cookie in enumerate(cookies, 1):
        print(f" {W}[{i}]{RESET} {M(cookie['name'])} (UID: {cookie['uid']})")
    print(LINE)
    selection = input(f" {W}[➤]{RESET} {C('Selection (ALL/1,2,3)')} {W}➤{RESET} ").strip().upper()
    if not selection: return None
    if selection == 'ALL': return cookies
    selected = []
    for part in selection.replace(',', ' ').split():
        try:
            index = int(part) - 1
            if 0 <= index < len(cookies): selected.append(cookies[index])
        except: pass
...
def select_tokens_for_sharing():
    ...
    selection = input(f" {W}[➤]{RESET} {C('Selection (ALL/1,2,3)')} {W}➤{RESET} ").strip().upper()
    if not selection: return None
    if selection == 'ALL': return tokens
    selected = []
    for part in selection.replace(',', ' ').split():
        try:
            index = int(part) - 1
            if 0 <= index < len(tokens): selected.append(tokens[index])
        except: pass
    return selected if selected else None
...
async def run_auto_share_v1(link, cookies):
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for account in valid_accounts:
            task = asyncio.create_task(share_worker_v1(session, account['cookie'], account['token'], post_id, account['uid'], display_mode))
            tasks.append(task)
        try:
...
async def run_auto_share_v2(link, tokens):
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for token_data in tokens:
            task = asyncio.create_task(share_worker_v2_fast(session, token_data['token'], post_id, token_data['uid'], display_mode))
            tasks.append(task)
        try:
            await asyncio.gather(*tasks)
        except:
...
def auto_share_v1():
    ...
    refresh_screen()
    print(f" {G('[SELECTED] ' + str(len(cookies)) + ' cookie(s)')}")
    print(LINE)
    link = input(f" {W}[➤]{RESET} {C('POST LINK/ID')}    {W}➤{RESET} ").strip()
    if not link: return
    try:
...
def auto_share_v2():
    ...
    refresh_screen()
    print(f" {G('[SELECTED] ' + str(len(tokens)) + ' token(s)')}")
    print(LINE)
    link = input(f" {W}[➤]{RESET} {C('POST LINK/ID')}    {W}➤{RESET} ").strip()
    if not link: return
    try:
...
def main():
    ...
    while True:
        ...
        try:
            ...
            sys.exit(0)
        if not user_token:
            if choice in ['1', '01', 'A']: do_login()
            elif choice in ['2', '02', 'B']: do_register()
            elif choice in ['0', '00', 'X']:
                print(f"\n {R('[!] Goodbye!')}")
                print(RESET_ALL)
                sys.exit(0)
        elif user_data and user_data.get('isAdmin'):
            if choice in ['1', '01', 'A']: auto_share_v1()
            elif choice in ['2', '02', 'B']: auto_share_v2()
            elif choice in ['3', '03', 'C']: manage_cookies()
            elif choice in ['4', '04', 'D']: manage_tokens()
            elif choice in ['5', '05', 'E']: show_stats()
            elif choice in ['6', '06', 'F']: admin_panel()
            elif choice in ['0', '00', 'X']: user_token = None; user_data = None
        elif user_data and user_data.get('plan') == 'maxplus':
            if choice in ['1', '01', 'A']: auto_share_v1()
            elif choice in ['2', '02', 'B']: auto_share_v2()
            elif choice in ['3', '03', 'C']: manage_cookies()
            ...
            if choice in ['1', '01', 'A']: auto_share_v1()
            elif choice in ['2', '02', 'B']: manage_cookies()
            elif choice in ['3', '03', 'C']: show_stats()
            elif choice in ['0', '00', 'X']: user_token = None; user_data = None
