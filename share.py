...
def get_gradient_color(progress, color_scheme='purple_cyan'):
    ...
    color_schemes = {
        'purple_cyan': [(138, 43, 226), (147, 51, 234), (168, 85, 247), (192, 132, 252), (216, 180, 254), (147, 197, 253), (125, 211, 252), (34, 211, 238)],
        'green_yellow': [(16, 185, 129), (52, 211, 153), (110, 231, 183), (167, 243, 208), (253, 224, 71), (250, 204, 21)],
        'red_orange': [(220, 38, 38), (239, 68, 68), (248, 113, 113), (251, 146, 60), (253, 186, 116), (254, 215, 170)],
        'blue_purple': [(37, 99, 235), (59, 130, 246), (96, 165, 250), (147, 197, 253), (167, 139, 250), (196, 181, 253)]
    }
...
def banner_header():
    ...
    if user_data:
        ...
        plan_display = M('[ MAX ]') if user_data.get('planExpiry') else M('[ MAX LIFETIME ]')
...
def login_user():
    ...
    if status == 200 and response.get('success'):
        ...
        print(f" {G('Plan:')} {M('MAX')}")
        print(f" {G('Total Cookies:')} {C(str(user_data.get('cookieCount', 0)))}")
        if user_data.get('planExpired'):
            print(LINE)
            print(f" {R('[WARNING] Your plan has expired!')}")
            print(f" {C('Contact admin to renew your subscription.')}")
...
def register_user():
    ...
    print(LINE)
    print(f" {C('[NOTE] This tool is PAID. Contact admin after registration.')}")
    print(f" {C('Pricing: 1 month (₱150) | 3 months (₱250) | LIFETIME')}")
    print(LINE)
    username = input(f" {W}[{W}➤{W}]{RESET} {C('USERNAME')} {W}➤{RESET} ").strip()
    if not username:
        return
    ...
    if status == 201 and response.get('success'):
        ...
        print(f" {G('• 1 Month  - ₱150')}")
        print(f" {G('• 3 Months - ₱250')}")
        print(f" {G('• LIFETIME - Contact admin')}")
        print(LINE)
    else:
...
def add_cookie():
    ...
    print(LINE)
    print(f" {C('You have UNLIMITED cookie storage.')}")
    print(LINE)
...
def admin_panel():
    while True:
        ...
        print(f" {W}[{W}2{W}]{RESET} {C('ACTIVATE USER PLAN')}")
        print(f" {W}[{W}3{W}]{RESET} {R('DELETE USER')}")
        print(f" {W}[{W}4{W}]{RESET} {G('DASHBOARD STATS')}")
        print(f" {W}[{W}0{W}]{RESET} {C('BACK')}")
        print(LINE)
        choice = input(f" {W}[{W}➤{W}]{RESET} {C('CHOICE')} {W}➤{RESET} ").strip()
        if choice == '1':
...
def view_all_users():
    ...
    if status == 200 and response.get('success'):
        ...
        for i, user in enumerate(users, 1):
            ...
            if user.get('planExpiry') is None:
                plan_status = M('LIFETIME')
            elif user.get('planExpiry') == 'Expired':
                plan_status = R('EXPIRED')
            else:
                plan_status = G(user.get('planExpiry'))
            print(f" {W}[{i:02d}]{RESET} {C(user['username'].upper())}{admin_badge}")
            print(f"      Facebook: {G(user.get('facebook', 'N/A'))}")
            print(f"      Plan: {M('MAX')} | Status: {plan_status}")
            print(f"      Country: {G(user['country'])} | Cookies: {C(str(user.get('cookieCount', 0)))}")
...
def activate_user_plan():
    ...
    for i, user in enumerate(users, 1):
        if user.get('planExpiry') is None:
            ...
            plan_status = G(user.get('planExpiry'))
        print(f" {W}[{i}]{RESET} {C(user['username'].upper())} - {plan_status}")
    print(LINE)
    ...
    print(f" {C(f'[ACTIVATE PLAN FOR: {username_display}]')}")
    print(LINE)
    print(f" {W}[1]{RESET} {M('1 MONTH')} - ₱150")
    print(f" {W}[2]{RESET} {M('2 MONTHS')} - ₱200")
    print(f" {W}[3]{RESET} {M('3 MONTHS')} - ₱250")
    print(f" {W}[4]{RESET} {M('LIFETIME')} - Permanent")
    print(LINE)
    plan_choice = input(f" {W}[{W}➤{W}]{RESET} {C('SELECT PLAN NUMBER')} {W}➤{RESET} ").strip()
    duration_map = {'1': 1, '2': 2, '3': 3, '4': None}
    if plan_choice not in duration_map:
        print(f" {R('[ERROR] Invalid plan')}")
        time.sleep(1)
        return
    duration = duration_map[plan_choice]
    refresh_screen()
    print(f" {C('[CONFIRM ACTIVATION]')}")
    print(LINE)
    print(f" User: {C(selected_user['username'].upper())}")
    if duration:
        print(f" Duration: {G(f'{duration} month(s)')}")
    else:
        print(f" Duration: {M('LIFETIME')}")
    print(LINE)
    confirm = input(f" {W}[{W}➤{W}]{RESET} {G('Confirm? (Y/N)')} {W}➤{RESET} ").strip().upper()
    if confirm != 'Y':
        return
    nice_loader("ACTIVATING")
    status, response = api_request("PUT", f"/admin/users/{selected_user['username']}/plan", {"duration": duration})
    if status == 200 and response.get('success'):
        print(f" {G('[SUCCESS] Plan activated successfully!')}")
    else:
        print(f" {R('[ERROR]')} {R(response.get('message', 'Failed to activate plan'))}")
    print(LINE)
    input(f"\n {G('[PRESS ENTER TO CONTINUE]')}")
...
def dashboard_stats():
    ...
    if status == 200 and response.get('success'):
        ...
        print(f" Active Users: {G(str(stats.get('activeUsers', 0)))}")
        print(f" Expired Users: {R(str(stats.get('expiredUsers', 0)))}")
        print(f" Lifetime Users: {M(str(stats.get('lifetimeUsers', 0)))}")
        print(LINE)
        ...
        for user in stats.get('recentUsers', []):
            lifetime_badge = M(' [LIFETIME]') if user.get('isLifetime') else ''
            print(f" {C(user['username'].upper())} - {G(user['country'])}{lifetime_badge}")
        print(LINE)
    else:
...
def file_encryptor():
    ...
    print(LINE)
    print(f" {C('Encrypt Python files with compression')}")
    print(LINE)
...
async def share_with_eaag(session, cookie, token, post_id):
    """Share with 3 retries for better success rate - NO DELAYS"""
    headers = {
        ...
        try:
            ...
            async with session.post(url, headers=headers, timeout=10) as response:
                ...
                if 'id' in json_data:
                    ...
                    last_error = json_data.get('error', {}).get('message', 'Unknown error')
        except Exception as e:
            last_error = str(e)
        retries -= 1
...
async def auto_share_main(link_or_id, selected_cookies):
    ...
    for cookie_data in selected_cookies:
        ...
        if token:
            eaag_tokens.append({'id': cookie_data['id'], 'cookie': cookie_data['cookie'], 'token': token, 'name': cookie_data['name'], 'uid': cookie_data['uid']})
            uid_text = f"UID: {cookie_data['uid']}"
            print(f" {G('✓')} {C(cookie_data['name'])} {W}({C(uid_text)}){RESET}")
        else:
    ...
    async with aiohttp.ClientSession() as session:
        ...
        print(f" {C('Retries:')} {M('3 retries per share')}")
        print(LINE)
        ...
        for acc in eaag_tokens:
            task = asyncio.create_task(share_loop(session, acc['cookie'], acc['token'], post_id, acc['name'], acc['uid'], acc['id'], display_mode))
            tasks.append(task)
...
def select_cookies_for_sharing():
    ...
    for i, cookie_data in enumerate(cookies, 1):
        ...
        uid_text = f"UID: {cookie_data['uid']}"
        print(f" {W}[{RESET}{BG_C}{W}{i:02d}{RESET}{W}/{RESET}{BG_C}{W}{letter}{RESET}{W}]{RESET} {C(cookie_data['name'])} {W}({C(uid_text)}){RESET}")
    print(LINE)
    ...
    for cookie_data in selected_cookies:
        uid_text = f"UID: {cookie_data['uid']}"
        print(f"   • {C(cookie_data['name'])} {W}({C(uid_text)}){RESET}")
    print(LINE)
    confirm = input(f" {W}[{W}➤{W}]{RESET} {G('Confirm? (Y/N)')} {W}➤{RESET} ").strip().upper()
    if confirm == 'Y':
...
def start_auto_share():
    ...
    print(f" {C('• Each share has 3 retries for better success')}")
